...
MODULE DATAMODULE
  ...
  ! set_outvar
  ! set_outvar_crit
  ! reform_input_data_for_pseudo_dayno
  ! change_cropdata_to_pseudo_dayno
  ! open_subbasinfiles
  ! close_subbasinfiles
...
CONTAINS
  ...
  !!
  !>\b Consequences Module modvar variables conduct, doupdate, updatestations, 
  ...
  SUBROUTINE prepare_for_update(dir,wobsvarname,locupall,locupnone,locupnone_qar,locupnone_war,wupall,wupnone,ns) 
    ...
    !Discharge data to use for update  (quseobs)
  ...
  FUNCTION get_current_flow(i,cd,n,no) RESULT(current_flow)
    ...
    IF(readformat==0.OR.readformat==4)THEN   !ASCII
      IF(readdaily)THEN
        ...
        ENDWHERE
        current_flow = y
        IF(d.NE.cd) WRITE(*,*) 'ERROR read Qobs, date not equal to Pobs date'   !borde ej kunna intr\E4ffa
      ELSE
  ...
  FUNCTION get_current_otherobs(i,cd) RESULT(current_obs)
    ...
    IF(readformat==0.OR.readformat==4)THEN   !ASCII
      ...
      IF(readdaily)THEN
        ...
        current_obs = y
        IF(d.NE.cd) WRITE(*,*) 'ERROR read Xobs, date not equal to Pobs date'   !borde ej kunna intr\E4ffa
      ELSE
  ...
  FUNCTION get_current_oregobs(i,cd) RESULT(current_obs)
    ...
    IF(readformat==0.OR.readformat==4)THEN   !ASCII
      ...
      IF(readdaily)THEN
        ...
        current_obs = y
        IF(d.NE.cd) WRITE(*,*) 'ERROR read Xoregobs, date not equal to Pobs date'   !borde ej kunna intr\E4ffa
      ELSE
  ...
  !----------------------------------------------------------------
  SUBROUTINE load_basindata(dir,fdir,lfile,infile,n,status) 
    ...
    DO i = 1, numslc
      CALL set_soildepth_classdata(NINT(slcdata(10,i)),slcdata(11:10+NINT(slcdata(10,i)),i),classdata(i)) !saved values from GeoClass.txt
      temp = set_soildepth(NINT(slcdata(10,i)),i,slcdata(11:10+NINT(slcdata(10,i)),i)) !initial values from GeoClass.txt
    ENDDO
    !Check vegtype>0
    vegtypereset = .FALSE.
    DO i = 1,nclass
    ...
    WRITE(6,*) 'Class information loaded'
    ...
    ELSE
      IF(SUM(wetland(1:n,1:2)%area)>0)THEN
        ...
        WRITE(6,*) 'Warning: River wetland will no be simulated.'
      ENDIF
      IF(ALLOCATED(wetland)) DEALLOCATE(wetland)
    ENDIF
  ...
  !------------------------------------------------------------------
  SUBROUTINE set_model_configuration(config)
  ...
  SUBROUTINE load_branchdata(dir,status) 
    ...
    DO i = 1,ncols
      ...
      IF(str(i)(1:10)=='qbranch   ') code(i) = i_intg
    ...
    DO i = 1,nrows
      ...
      IF(branchsubid(i)%mainpart<0. .OR. branchsubid(i)%mainpart>1.)THEN
        ...
        STOP 1
      ENDIF
      IF(branchsubid(i)%recQbranch)THEN
  ...
  SUBROUTINE load_glacierdata(dir,nsb,status) 
    ...
    DO i = 1,ncols
      ...
      IF(str(i)(1:10)=='slcvolume ') code(i) = i_real
    ENDDO
    ...
    DO i = 1,ncols
      ...
      IF(str(i)(1:10)=='slcvolume ') glacier(1:nrows)%inivol = xr(1:nrows,rindex(i))
    ENDDO
  ...
  SUBROUTINE load_lakedata(funit,dir,n,lakedataid,status) 
    ...
    DO i = 1,ncols
      ...
      IF(str(i)(1:10)=='obsflow   ') code(i) = i_intg   !flag for wanted bifurcation flow
    ...
    DO j = 1,nrows
      ...
      ENDIF
    ENDDO
    IF(ilake<nlakedim)THEN
      WRITE(6,*) 'Warning: Some lakes in LakeData is not coupled to subbasins in GeoData.'
      WRITE(6,*) 'Warning: LakeData has',nlakedim,' lake(s), while GeoData has',ilake,' lake(s).'
    ENDIF
  ...
  SUBROUTINE load_flooddata(funit,dir,n,status)
    ...
    DO i = 1,ncols
      ...
      IF(str(i)(1:10)=='hrefr     ')THEN
        code(i) = i_real
        hrefstatus = .TRUE.
      ENDIF
      IF(str(i)(1:10)=='hrefl     ')THEN
        code(i) = i_real
        hrefstatus = .TRUE.
      ENDIF
    ENDDO
    ...
    DO idam = 1,nrows
      DO i = 1,ncols
        ...
        IF(str(i)(1:10)=='hrefr     ') flooding(idam)%hrefr  = xr(idam,rindex(i))
        IF(str(i)(1:10)=='hrefl     ') flooding(idam)%hrefl  = xr(idam,rindex(i))
      ENDDO
    ENDDO
...
!                  lakedatapar(1:nlakeregions, modparid(ldparCounter)%parno) = lregpar(modparid(parCounter)%parno,1:nlakeregions)
  ...
  SUBROUTINE read_and_calc_basindata(funit,infile,n,lakedataid,maxcol,mcols) 
    ...
    DO i = 1,mcols
      ...
      IF(str(i)(1:10)=='mainfl    ') WRITE(6,*) 'WARNING: mainfl in GeoData is an column name no longer used' 
      ...
      IF(str(i)(1:10)=='wetdep_n  ')THEN
        code(i) = i_real
        wetdep_nfound = .TRUE.  ! For deallocating deposition%inwetconc if not used
      ENDIF
      IF(str(i)(1:10)=='drydep_n1 ')THEN
        code(i) = i_real
        drydep_nfound = .TRUE.  ! For deallocating deposition%indryload if not used
      ENDIF
      IF(str(i)(1:10)=='drydep_n2 ')THEN
        code(i) = i_real
        drydep_nfound = .TRUE.  ! For deallocating deposition%indryload if not used
      ENDIF
      IF(str(i)(1:10)=='drydep_n3 ')THEN
        code(i) = i_real
        drydep_nfound = .TRUE.  ! For deallocating deposition%indryload if not used
      ENDIF
      IF(str(i)(1:10)=='deploadn1 ') THEN
        code(i) = i_real
        deploadnfound = .TRUE.  ! For deallocating deposition% if total IN load on water is not used
      ENDIF
      IF(str(i)(1:10)=='deploadn2 ') code(i) = i_real
      IF(str(i)(1:10)=='deploadn3 ') code(i) = i_real
      IF(str(i)(1:10)=='deploadn4 ') code(i) = i_real
      IF(str(i)(1:10)=='deploadn5 ') code(i) = i_real
      IF(str(i)(1:10)=='deploadn6 ') code(i) = i_real
      IF(str(i)(1:10)=='deploadn7 ') code(i) = i_real
      IF(str(i)(1:10)=='deploadn8 ') code(i) = i_real
      IF(str(i)(1:10)=='deploadn9 ') code(i) = i_real
      IF(str(i)(1:10)=='deploadn10') code(i) = i_real
      IF(str(i)(1:10)=='deploadn11') code(i) = i_real
      IF(str(i)(1:10)=='deploadn12') code(i) = i_real
    ...
    DO i = 1,mcols
      ...
      IF(str(i)(1:10)=='wetdep_n  ')  deposition%inwetconc(1:n) = xr(1:n,rindex(i))*1.E-3    !ug/L -> mg/L
      IF(str(i)(1:10)=='drydep_n1 ')  deposition%indryload(1:n,1) = xr(1:n,rindex(i))*onetspday  !kg/km2/d->kg/km2/ts
      IF(str(i)(1:10)=='drydep_n2 ')  deposition%indryload(1:n,2) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='drydep_n3 ')  deposition%indryload(1:n,3) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='deploadn1 ')  deposition%inloadwater(1:n,1) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='deploadn2 ')  deposition%inloadwater(1:n,2) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='deploadn3 ')  deposition%inloadwater(1:n,3) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='deploadn4 ')  deposition%inloadwater(1:n,4) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='deploadn5 ')  deposition%inloadwater(1:n,5) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='deploadn6 ')  deposition%inloadwater(1:n,6) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='deploadn7 ')  deposition%inloadwater(1:n,7) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='deploadn8 ')  deposition%inloadwater(1:n,8) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='deploadn9 ')  deposition%inloadwater(1:n,9) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='deploadn10')  deposition%inloadwater(1:n,10) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='deploadn11')  deposition%inloadwater(1:n,11) = xr(1:n,rindex(i))*onetspday
      IF(str(i)(1:10)=='deploadn12')  deposition%inloadwater(1:n,12) = xr(1:n,rindex(i))*onetspday
    ...
    DO i = 1,n
      ...
      ENDDO
      IF(numsubstances>0) load(i)%psload(:,:) = 0.
      IF(simulate%substance(i_in).AND.simulate%substance(i_on))THEN
        DO k = 1,max_pstype
        ...
        ENDDO
      ENDIF
      IF(simulate%substance(i_sp).AND.simulate%substance(i_sp))THEN
        DO k = 1,max_pstype
  ...
  !>
  !>Soil nutrient leakage concentration is read from file and replaces the 
  !>concentration calculated by HYPE soil routines. The runoff from classes 
  !>gets the new concentration.
  !!
  !>\b Consequences Module modvar variable soilleak is allocated and set. 
  !>
  !>\b Reference ModelDescription Chapter X (XXX)
  !------------------------------------------------------------------------------
  SUBROUTINE load_soilleakage_concentrations(dir,n,status) 
    ...
    !Or read other soilleakage
    ELSEIF(modeloption(p_soilleakage)==2)THEN
    ENDIF
    
    !Finish the subroutine
    IF(ALLOCATED(geoid)) DEALLOCATE(geoid)
    WRITE(6,*) 'Soil leakage concentration information loaded'

  END SUBROUTINE load_soilleakage_concentrations

  !>\brief Reads the matrix of monthly soil leakage concentrations
  !>
  !>Reads the montly soil leakage concentrations and checks them.
  !>
  !> \b Reference ModelDescription Chapter X (XXX)
  !------------------------------------------------------------------------------
  SUBROUTINE read_monthly_leakage(dir,infile,n,geoid,status,leakage) 
    ...
    IF(status/=0)RETURN
    IF(n/=nrows)THEN
      WRITE(6,*) 'ERROR: Number of rows in LeakageData not equal to number of subbasins.'
      status = 1
      RETURN
    ENDIF
    IF(ncols<13)THEN
    ...
    IF(MINVAL(readleakage)<0.) WRITE(6,*) 'WARNING: Concentration less than zero found in file.'
  ...
  SUBROUTINE load_management_data(funit,dir,n,status) 
    ...
    DO i = 1,ncols
      ...
      IF(str(i)(1:10)=='waterbody ') code(i) = i_intg
      IF(str(i)(1:10)=='receiver  ') code(i) = i_intg
      IF(str(i)(1:10)=='flow      ') code(i) = i_real
    ENDDO
    ...
    ELSE
      ...
      nwtransfer=0
    ENDIF
!    IF(nwtransfer>0) dowatertransfer = .TRUE.
      ...
      DO i = 1,ncols
        ...
        DO j = 1,nrows
          IF(xi(j,iindex(typecolumn))==1)THEN !Irrigation
            irrow = irrow + 1
            IF(str(i)(1:10)=='subid     ') irrigationsystem(irrow)%subid       = xi(j,iindex(i))
            IF(str(i)(1:10)=='regsrcid  ') irrigationsystem(irrow)%regsourceid = xi(j,iindex(i))
            IF(str(i)(1:10)=='local_eff ') irrigationsystem(irrow)%local_eff   = xr(j,rindex(i))
            IF(str(i)(1:10)=='region_eff') irrigationsystem(irrow)%reg_eff     = xr(j,rindex(i))
            IF(str(i)(1:10)=='demandtype') irrigationsystem(irrow)%demandtype  = xi(j,iindex(i))
            IF(str(i)(1:10)=='gw_part   ')THEN
          ...
          ELSEIF(xi(j,iindex(typecolumn))==2)THEN !Water transfer
            itrow = itrow + 1
            IF(str(i)(1:10)=='subid     ') watertransfer(itrow)%srcsubid = xi(j,iindex(i))
            IF(str(i)(1:10)=='flow      ') watertransfer(itrow)%flow     = xr(j,rindex(i))
            IF(str(i)(1:10)=='receiver  ')THEN
              DO isub = 1,n
...
!    ENDIF
  ...
  SUBROUTINE load_cropdata(dir,n,status) 
    ...
    calcNP = simulate%substance(i_in).OR.simulate%substance(i_on).OR.simulate%substance(i_sp).OR.simulate%substance(i_pp)
    ...
    IF(.NOT.fileexist)THEN
      IF(calcNP)THEN
        ...
        status = 1
      ENDIF
      IF(simulate%substance(i_ss).AND. modeloption(p_erosion)==0)THEN
...
300 WRITE(*,*) 'ERROR reading filedir.txt' 
  ...
  !> readobsid, writeload, readmatlab, writematlab,
  ...
  SUBROUTINE load_coded_info(dir,status,date1,date2,date3,skip,step,nsubst,     &
                             ...
                             mdir,rdir,fdir,                           &
                             locupall,locupnone,locupnone_qar,locupnone_war, &
                             wupall,wupnone,wupdname,noutvar,subincrit) 
  ...
  USE READWRITE_ROUTINES, ONLY : convert_string_to_integer, &
    ...
    DO
      ...
      IF(code(1:10)=='readformat' .OR. &
        ...
        ELSEIF(code(1:11)=='indatacheck')THEN
          checkindata = intvalue
        ENDIF
        CYCLE
      ENDIF
...
200 CONTINUE
    ...
    !IF(.NOT.fdirset) fdir = mdir
    CALL handle_relative_directories(dir,mdir,rdir,fdir)
    IF(.NOT.readdaily)THEN   
      IF(step>26*366)THEN
        WRITE(6,*)
    ...
    WRITE(6,*)'     River wetland model',modeloption(p_wetland)
    ...
    ENDIF
    DO i=1,sonum
      WRITE(6,*)'State variables will be saved for: ',strstates(i)
    ENDDO
    WRITE(6,*)'-------------------------------------------------'
    WRITE(6,*)
...
801 CONTINUE
  ...
  !------------------------------------------------------------------------
  SUBROUTINE handle_relative_directories(dir,mdir,rdir,fdir)
  ...
  SUBROUTINE load_submodel_info(dir,submodelrun,msub,status)
    ...
    IF(submodelrun)THEN
      WRITE(6,*) 'File opened:',TRIM(filename)
      OPEN(UNIT = fileunit_temp,FILE = filename, STATUS = 'old', ACTION='read')
      READ(fileunit_temp,*,ERR=200) msub
      IF(.NOT.ALLOCATED(selectedaro)) ALLOCATE(selectedaro(msub))
      READ(fileunit_temp,*,ERR=200) selectedaro
      CLOSE(fileunit_temp)
    ELSE
      msub = nsub_basemodel
    ENDIF
...
200 CONTINUE
  ...
  USE MODVAR, ONLY : CROPDATATYPE,  &
    ...
    IF(ALLOCATED(cropdata))THEN
      IF(cropdata(row)%fertday1>0) cropdata(row)%fertday1 = get_pseudo_dayno(cropdata(row)%fertday1,-1.)
      IF(cropdata(row)%fertday2>0) cropdata(row)%fertday2 = get_pseudo_dayno(cropdata(row)%fertday2,-1.)
      IF(cropdata(row)%manday1>0) cropdata(row)%manday1 = get_pseudo_dayno(cropdata(row)%manday1,-1.)
      IF(cropdata(row)%manday2>0) cropdata(row)%manday2 = get_pseudo_dayno(cropdata(row)%manday2,-1.)
      ...
      IF(cropdata(row)%firstday>0) cropdata(row)%firstday = get_pseudo_dayno(cropdata(row)%firstday,-1.)
    ENDIF
    IF(ALLOCATED(cropirrdata))THEN
      IF(cropirrdata(row)%plantingdayno>0) cropirrdata(row)%plantingdayno = get_pseudo_dayno(cropirrdata(row)%plantingdayno,-1.)
  ...
  SUBROUTINE write_subbasin_assessment(filedir,numsub,na,sas2,iens,runens)
    ...
    DO ia = 1, na
      ...
      IF(tal==':') tal = 'A'      !f\F6r fler kriterier \E4n 9 anv\E4nds bokst\E4ver
  ...
  SUBROUTINE close_subbasinfiles(n,iens)
    ...
    DO io=1,noutput
      IF(output(io)%fileformat==1)THEN
        ...
        IF(output(io)%narea>=0) nselected = output(io)%narea
        DO i = 1,nselected
          funit=fileunit_add_sub+n*(io-1)+i+(iens-1)*fileunit_add_dabout   !denna funit beh\F6ver ocks\E5 io n\E4r subbasinfiler blir fler
          CLOSE(funit)
        ENDDO
      ELSEIF(output(io)%fileformat==4)THEN
        nselected = noutreg
    ...
    DO 
      ...
      ELSE
        ...
        CASE(m_spar)
		  ...
		  !write(50001,*)"varstr=",varstr,"modparid(varindex)%parno=",values(1:nvalues)
        CASE(m_lpar)
...
100 CONTINUE
    CLOSE(fileunit_temp)
    WRITE(6,*) 'Model parameters loaded (par.txt)'
...
100 WRITE(6,*) 'End of file during read optpar.txt for river dimension' 
  ...
  SUBROUTINE checkindata_part1()
    ...
    IF(subidproc>max_subid)THEN
      WRITE(6,*) 'Error: Subid is too large. Maximum ',max_subid,' may be used. Check GeoData.txt.'
      checkdata(1,2)=.TRUE.
    ENDIF
