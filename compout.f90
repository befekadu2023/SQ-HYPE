...
!with HYPE. If not, see <http://www.gnu.org/licenses/>.
  ...
  SUBROUTINE calculate_upstream_average(varindex,areatype,avvalue)
    ...
    DO isb=1,nsub
      ...
      IF(areatype==3.AND.outvar(isb,varindex)>0.) avvalue(isb) = avvalue(isb) + outvar(isb,varindex)*1.E3/upstreamarea(isb)
      IF(areatype==4.AND.outvar(isb,varindex)>0.) avvalue(isb) = avvalue(isb) + outvar(isb,varindex)*1.E12/upstreamarea(isb)
      IF(areatype==5.AND.outvar(isb,varindex)>0.) avvalue(isb) = avvalue(isb) + outvar(isb,varindex)*1.E9/upstreamarea(isb)
      IF(areatype==6.AND.outvar(isb,varindex)>0.) avvalue(isb) = avvalue(isb) + outvar(isb,varindex)*1.E6/upstreamarea(isb)
...
!    CALL scalar_lower_case(str2)
  ...
  SUBROUTINE accumulate_periodvalues(m,n,r,c,rs,cs,ts)
    ...
    !WHERE(r(:)/=m .AND. c(:)/=m)																		!!Commented by BTW
    !  rs(:) = rs(:) + r(:)
    !  cs(:) = cs(:) + c(:)
    !  ts(:) = ts(:) + 1
    !ENDWHERE
	WHERE(r(:)/=m .AND. c(:)/=m .AND. r(:)/=0.0 .AND. c(:)/=0.)											!!Modified by BTW
      rs(:) = rs(:) + log(r(:))																			!!Modified by BTW
      cs(:) = cs(:) + log(c(:))																			!!Modified by BTW
      ts(:) = ts(:) + 1																					!!Modified by BTW
    ENDWHERE
  ...
  SUBROUTINE calculate_fit(n,c,r,m,lim,x)
    ...
    DO i = 1,n
      !IF(r(i) == m .OR. c(i) == m) THEN												!!Commented by BTW
      !ELSE
      !  nobs = nobs+1
      !  sumc = sumc+DBLE(c(i))
       ! sumr = sumr+DBLE(r(i))
      ...
      !ENDIF
	  IF(r(i) == m .OR. c(i) == m .OR. r(i) == 0.0 .OR. c(i) == 0.) THEN												!!Added by BTW
      ELSE																												!!Added by BTW
        nobs = nobs+1																									!!Added by BTW
        sumc = sumc+DBLE(log(c(i)))																						!!Added by BTW
        sumr = sumr+DBLE(log(r(i)))																						!!Added by BTW
        sumr2 = sumr2+DBLE(log(r(i)))**2																				!!Added by BTW
        sume2 = sume2+(DBLE(log(c(i))-log(r(i))))**2   !Errors^2														!!Added by BTW
      ENDIF																												!!Added by BTW
    ENDDO
