...
!IRRIGATION_MODULE, REGIONAL_GROUNDWATER_MODULE
  ...
  SUBROUTINE model_version_information(funit)
    ...
    WRITE(funit,600) '----------------------'
    WRITE(funit,600) ' HYPE version 5.5.1  '
    WRITE(funit,600) '----------------------'
600 FORMAT(A22)
  ...
  SUBROUTINE define_output_variables()
    ...
    max_outvar = 360     !number of output variables
    ALLOCATE(outvarid(max_outvar))
    
    outvarid(o_crun) = outvaridtype('crun','computed runoff', i_sum , 0,2,'mm' ,'mm' ,'mm')
    outvarid(o_rrun) = outvaridtype('rrun','recorded runoff', i_sum , 0,0,'mm' ,'mm' ,'')
    outvarid(o_prec) = outvaridtype('prec','precipitation'  , i_sum , 0,1,'mm' ,'mm' ,'mm')
    outvarid(o_tobs)  = outvaridtype('temp','air temperature'   ,i_mean , 0,1,'deg'  ,'degree Celsius','deg')
    ...
    outvarid(o_soim) = outvaridtype('soim','computed soil water',i_mean, 0,2,'mm' ,'mm' ,'mm')
    ...
    outvarid(o_snow) = outvaridtype('snow','snow pack'         ,i_mean , 0,2,'mm' ,'mm water' ,'mm')
    outvarid(o_evap) = outvaridtype('evap','subbasin evaporation',i_sum , 0,1,'mm'   ,'mm' ,'mm')
    ...
    outvarid(o_grwlevel) = outvaridtype('gwat','computed grw level',i_mean , 0,2,'m' ,'m' ,'m')
    ...
    outvarid(o_soilfrost) = outvaridtype('sfst','computed soil frost' ,i_mean , 0,2, 'cm'  ,'cm' ,'cm')
    outvarid(o_soiltmp) = outvaridtype('stmp','computed soil temp'  ,i_mean , 0,2, 'deg' ,'degree Celcius','deg')
    outvarid(o_snowdepth) = outvaridtype('sdep','computed snow depth' ,i_mean , 0,2, 'cm'  ,'cm' ,'cm')
    outvarid(o_epot) = outvaridtype('epot','potential evap'      ,i_sum  , 0,1, 'mm'  ,'mm' ,'mm')
    ...
    outvarid(43) = outvaridtype('cprc','corr precipitation'     ,i_sum , 0,1,'mm'   ,'mm' ,'mm')
    ...
    outvarid(47) = outvaridtype('phC1','pool humusC soil1',i_mean , 0,2,'kg/km2' ,'kg org-C/km2','kg/km2')
    outvarid(48) = outvaridtype('pfC1','pool fastC soil1',i_mean , 0,2,'kg/km2' ,'kg org-C/km2','kg/km2')
...
!    outvarid(o_upte) = outvaridtype('upte','aver upstr temperature',i_mean, 0,0,'deg','degree Celsius' ,'mapUPTE.txt','timeUPTE.txt')
    ...
    outvarid(o_pfN(1)) = outvaridtype('pfN1','pool fastN soil1' ,i_mean , 0,2,'kg/km2' ,'kg N/km2','kg/km2')
    ...
    outvarid(o_phN(1)) = outvaridtype('phN1','pool humusN soil1',i_mean , 0,2,'kg/km2' ,'kg N/km2','kg/km2')
    ...
    outvarid(o_psoilIN(1)) = outvaridtype('pIN1','pool InorgN soil1',i_mean , 0,2,'kg/km2' ,'kg N/km2','kg/km2')
    ...
    outvarid(o_ro1) = outvaridtype('cro1','computed runoff 1',i_sum  , 0,2,'mm'     ,'mm' ,'mm')
    ...
    outvarid(o_rod) = outvaridtype('crod','computed rf drain',i_sum  , 0,2,'mm' ,'mm' ,'mm')
    outvarid(o_ros) = outvaridtype('cros','computed surface rf'    ,i_sum ,0,2,'mm' ,'mm' ,'mm')
    outvarid(o_soildenitr) = outvaridtype('deni','denitrifikation'  ,i_sum , 0,2,'kg/km2' ,'kg N/km2','kg/km2')
    outvarid(o_cropNupt) = outvaridtype('crut','crop uptake'      ,i_sum , 0,2,'kg/km2' ,'kg N/km2','kg/km2')
    outvarid(o_degrfN) = outvaridtype('faIN','fast to inorganic',i_sum , 0,2,'kg/km2' ,'kg N/km2','kg/km2')
    outvarid(o_soilNatm) = outvaridtype('atmd','atm deposition N' ,i_sum , 0,2,'kg/km2' ,'kg N/km2','kg/km2')
    outvarid(o_ppP(1)) = outvaridtype('ppP1','pool partP soil1',i_mean , 0,2,'kg/km2' ,'kg P/km2','kg/km2')
    ...
    outvarid(o_psoilSP(1)) = outvaridtype('pSP1','pool SRP soil1',  i_mean , 0,2,'kg/km2' ,'kg P/km2','kg/km2')
    ...
    outvarid(o_pfP(1)) = outvaridtype('pfP1','pool fastP soil1',i_mean , 0,2,'kg/km2' ,'kg P/km2','kg/km2')
    ...
    outvarid(114) = outvaridtype('clrv','comp local river volume',i_mean , 0,3,'m3' ,'m3' ,'mm')
    outvarid(115) = outvaridtype('cmrv','comp main river volume', i_mean , 0,3,'m3' ,'m3' ,'mm')
    outvarid(o_soilPatm) = outvaridtype('atmp','atm deposition TP', i_sum , 0,2,'kg/km2' ,'kg P/km2','kg/km2')
    outvarid(117) = outvaridtype('glcv','comp glacier ice volume', i_mean , 0,4,'km3' ,'km3','mm')
    outvarid(118) = outvaridtype('glca','comp glacier area', i_mean , 0,6,'km2' ,'km2','-')
    ...
    outvarid(o_ctmp) = outvaridtype('ctmp','corrected air temperature'   ,i_mean , 0,1,'deg'  ,'degree Celsius','deg')
    outvarid(122) = outvaridtype('irel','irrigation evap losses'   ,i_sum , 0,3,'m3'  ,'m3' ,'mm') 
    outvarid(o_coum) = outvaridtype('coum','comp outflow main',i_mean , 0,0,'m3/s','m3/s','')
    outvarid(124) = outvaridtype('irld','abstr local dam f irr'    ,i_sum , 0,3,'m3'  ,'m3' ,'mm')
    outvarid(125) = outvaridtype('irlr','abstr local river f irr'  ,i_sum , 0,3,'m3'  ,'m3' ,'mm')
    outvarid(o_applirr) = outvaridtype('irra','applied irrigation water' ,i_sum , 0,3,'m3'  ,'m3' ,'mm')
    outvarid(127) = outvaridtype('irrg','gwater abstracted f irr'  ,i_sum , 0,3,'m3'  ,'m3' ,'mm')
    outvarid(128) = outvaridtype('irrs','irr abstr f other subbasins'   ,i_sum , 0,3,'m3'  ,'m3' ,'mm')
    outvarid(o_crun2) = outvaridtype('loff','computed runoff',i_mean, 0,0,'L/km2s','L/km2/s','')
    outvarid(o_coub) = outvaridtype('coub','comp outflow branch',i_mean , 0,0,'m3/s','m3/s','')
    outvarid(o_soildef) = outvaridtype('smdf','soil moisture deficit'    ,i_mean , 0,2,'mm','mm' ,'mm')
    outvarid(o_phP(1)) = outvaridtype('phP1','pool humusP soil1',i_mean , 0,2,'kg/km2' ,'kg N/km2','kg/km2')
    ...
    outvarid(o_psoilON(1)) = outvaridtype('pON1','pool orgN soil1',i_mean , 0,2,'kg/km2' ,'kg N/km2','kg/km2')
    ...
    outvarid(o_sltmp(1)) = outvaridtype('stm1','computed soillayer 1 temp'  ,i_mean , 0,2, 'deg' ,'degree Celcius','deg')
    ...
    outvarid(o_rainfall) = outvaridtype('cpRF','precipitation as rain'     ,i_sum , 0,1,'mm','mm' ,'mm')
    outvarid(o_snowfall) = outvaridtype('cpSF','precipitation as snow'     ,i_sum , 0,1,'mm','mm' ,'mm')
    outvarid(144) = outvaridtype('colv','volume of olake/w',i_mean , 0,5,'M(m3)' ,'M(m3)' ,'mm') !Computed Lake Volume  (basins summed to outlet if any) 
    outvarid(145) = outvaridtype('cilv','volume of ilake',i_mean , 0,5,'M(m3)' ,'M(m3)' ,'mm') !Computed ILake Volume 
    outvarid(146) = outvaridtype('clbv','volume of olake/lb',i_mean , 0,5,'M(m3)' ,'M(m3)' ,'mm') !Computed Olake Volume Computed (volumes for individual basins if any)
    ...
    outvarid(o_snowmax) = outvaridtype('smax','comp snowmax in winter'   ,i_mean , 0,2,'mm' ,'mm' ,'mm')
    ...
    outvarid(o_landevap) = outvaridtype('levp','land evaporation'        ,i_sum  , 0,2,'mm' ,'mm' ,'mm')
    outvarid(196) = outvaridtype('som2','soil water upper 2 l',i_mean , 0,2,'mm' ,'mm' ,'mm')
    outvarid(o_sml1) = outvaridtype('sml1','soil moisture lay 1' ,i_mean , 0,2,'mm' ,'mm' ,'mm')
    ...
    outvarid(200) = outvaridtype('stsw','standing soil water' ,i_mean , 0,2,'mm' ,'mm' ,'mm')
    outvarid(201) = outvaridtype('smrz','soil moisture root z',i_mean , 0,2,'mm' ,'mm' ,'mm')
    outvarid(202) = outvaridtype('sm13','soil moisture sl 1-3',i_mean , 0,2,'mm' ,'mm' ,'mm')
    ...
    outvarid(o_icloss) = outvaridtype('icpe','interception loss'    ,i_sum , 0,1,'mm' ,'mm' ,'mm')
    ...
    outvarid(240) = outvaridtype('aqin','aquifer recharge'   ,i_sum , 0,3,'m3'  ,'m3' ,'mm')
    outvarid(241) = outvaridtype('aqut','aquifer return flow',i_sum , 0,3,'m3'  ,'m3' ,'mm')
    ...
    outvarid(260) = outvaridtype('S111','fsusnow mean density open',i_mean, 0,0,'g/cm3','g/c8m3','')
    ...
    outvarid(o_ros1) = outvaridtype('ros1','comp. sat surface rf'    ,i_sum ,0,2,'mm' ,'mm' ,'mm')
    outvarid(o_ros2) = outvaridtype('ros2','comp. excess infilt'    ,i_sum ,0,2,'mm' ,'mm' ,'mm')
!    outvarid(o_uppe) = outvaridtype('uppe','aver upstream pot evap',i_sum, 0,0,'mm','mm','mapUPPE.txt','timeUPPE.txt')
    outvarid(o_evapsnow) = outvaridtype('evsn','snow evaporation',i_sum , 0,1,'mm' ,'mm' ,'mm')
    outvarid(279) = outvaridtype('evpt','total evaporation',i_sum , 0,1,'mm' ,'mm' ,'mm')
    ...
    outvarid(o_psim) = outvaridtype('psim','sim. precipitation', i_sum , 0,1,'mm','mm' ,'mm')
    ...
    outvarid(o_sml9) = outvaridtype('sml9','soil water layer 1' ,i_mean, 0,2,'mm','mm' ,'mm')
    outvarid(o_snowmelt) = outvaridtype('melt','snow melt' ,i_mean, 0,2,'mm','mm' ,'mm')
    ...
    outvarid(o_dwtr) = outvaridtype('dwtr','demand water transfer',i_mean, 0,0,'m3/s' ,'m3/s','')
    outvarid(o_rpwl) = outvaridtype('rpwl','m river flpl w level' ,i_mean , 0,0,'m'  ,'m in FD-href','')
    outvarid(o_lpwl) = outvaridtype('lpwl','olake flpl w level'   ,i_mean , 0,0,'m'  ,'m in FD-href','')
    outvarid(o_gmlt) = outvaridtype('gmlt','glacier melt'   ,i_mean , 0,0,'mm'  ,'mm' ,'')

    outvarid(o_mrfa) = outvaridtype('mrfa','main river fractional area'   ,i_mean , 0,0,'-'  ,'fraction of river area','')
    outvarid(o_lrfa) = outvaridtype('lrfa','local river fractional area'   ,i_mean , 0,0,'-'  ,'fraction of river area','')

    outvarid(o_mred) = outvaridtype('mred','main river effective depth'   ,i_mean , 0,0,'m'  ,'meter','')
    outvarid(o_lred) = outvaridtype('lred','local river effective depth'   ,i_mean , 0,0,'m'  ,'meter','')
    ...
    loadheadings=['subid ','WetAtm','DryAtm','Fertil','PDecay','RuralA','GrwSIn','IrrSrc','Runoff','RuralB','Urban1','Urban2','Urban3','Rgrwmr','Wtrans','Rgrwol','A     ','B     ','C     ','D     ','E     ','F     ','G     ','H     ','I     ','J     ','K     ','L     ','M     ','N     ']
  ...
  !>modparid structure: name, dependence type, index (defined in hypevar)
    ...
    max_par = 324          !maximum number of parameters
    ...
    modparid(322)  = modparidtype('wetdepspl ', m_gpar , m_wetspload)   !Only used on water
...
!------------------------------------------------------------
  ...
  SUBROUTINE initiate_model_state(frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
    ...
    CALL initiate_river_npc_state(conduct%simN,conduct%simP,conduct%simC,conduct%simS,i_t1>0,riverstate)
    
    !Initialize lake concentrations
    CALL initiate_lake_npc_state(simulatesubstances,conduct%simN,conduct%simP,conduct%simC,conduct%simS,slc_ilake>0,slc_olake>0,lakestate)
...
!                               reinitiate_soil_depth
    ...
    ELSE
      ...
      DO i = 1,nsub
        ...
        ENDIF
        IF(part>1.)THEN
          IF(conductwarning) WRITE(6,*) 'Warning: damping parameter larger than one. Set to one.'
          part = 1.
        ELSEIF(part<0.)THEN
          IF(conductwarning) WRITE(6,*) 'Warning: damping parameter less than zero. Set to zero.'
          part = 0.
        ENDIF
        DO itype = 1,2          
    ...
    IF(conduct%simN.OR.conduct%simP) CALL set_Qvariables_for_bankfulflow(nsub,riverstate%Q365)

    !Initialize lake parameters
    IF(simulatesubstances) CALL set_lake_slowwater_maxvolume(nsub,basin%lakedepth(1), &
        basin(:)%lakedepth(2),lakedataparindex,lakedatapar(:,m_lddeeplake),slc_ilake>0,slc_olake>0)
  ...
  SUBROUTINE model(frozenstate,soilstate,aquiferstate,riverstate,lakestate,miscstate)
    ...
    REAL classheight          !class height (m\F6h)
    REAL lakeareakm2
    ...
    IF(modeloption(p_floodplain)>=3)THEN
      IF(conduct%floodplain) CALL calculate_regional_floodplain_flows(nsub,miscstate,floodplaindammedflowtoolake,downstreamdammedflowonolake,downstreamdammedflowonmainriver)
    ENDIF
    ...
    DO i = 1,nsub
      ...
      DO j=1,nclass
        ...
        classheight = basin(i)%elev+classbasin(i,j)%deltah  !m\F6h
        IF(a>0)THEN
          ...
          IF(simulate%substance(i_t1)) CALL calculate_class_outvar_add(o_cevapT1,i,aadj,cevap(i_t1)*evap)  !accumulation of amount
          ...
          ENDIF
          IF(simulate%substance(i_t2)) CALL calculate_class_outvar_add(o_csoilT2,i,aadj,soilstate%conc(i_t2,1,j,i)*soilstate%water(1,j,i) + soilstate%conc(i_t2,2,j,i)*soilstate%water(2,j,i) + soilstate%conc(i_t2,3,j,i)*soilstate%water(3,j,i))
          !Soil load output, mm*mg/L
          IF(simulatesubstances.AND.SUM(outvarindex(285:322))>0)THEN
          ...
          CALL calculate_class_outvar_add(o_crun2,i,aadj,runofflows(1)+runofflows(2)+runofflows(3)+totalsurfaceflow+runoffd)
          IF(simulate%substance(i_t1)) CALL calculate_class_outvar_add(o_crunT1,i,aadj,crunoffi(i_t1))
          IF(simulate%substance(i_t2)) CALL calculate_class_outvar_add(o_crunT2,i,aadj,crunoffi(i_t2))
          IF(conduct%simN)THEN
          ...
          ENDIF
          IF(simulate%substance(i_ss)) CALL calculate_class_outvar_add(o_crunSS,i,aadj,crunoffi(i_ss))
          IF(simulate%substance(i_oc)) CALL calculate_class_outvar_add(o_crunOC,i,aadj,crunoffi(i_oc))
...
!      CALL calculate_class_outvar_finish(o_csoillayerIN(3),i,soilwateri(3)*1.E-3)
      DO isl = 1,3
        ...
        IF(modeloption(p_soilleakage)==0)THEN
          CALL calculate_class_outvar_finish(o_csoillayerIN(isl),i,soilwateri(isl)*1.E-3)  !inorgN conc soillayer (ug/L)
      ...
      CALL calculate_class_outvar_finish_scale(o_crun2,i,1.E6/seconds_per_timestep)
      ...
      IF(modeloption(p_soilleakage)>0)THEN
        IF(conductload)THEN
          ...
          Lrurala = 0.
          Lgrwsoil = 0.
          Latmdep = 0.  !will be set below
          Lstream = 0.  !will be set below
        ENDIF
        crunofftsi = get_current_soil_leakage(i,numsubstances)*runofftsi  !mg/L*mm
        !Set output runoff concentration
        IF(simulate%substance(i_t1))THEN
          CALL calculate_class_outvar_initialize(o_crunT1,i)
          CALL calculate_class_outvar_add(o_crunT1,i,1.,crunofftsi(i_t1))
          CALL calculate_class_outvar_finish(o_crunT1,i,runofftsi)  !T1 conc in runoff
        ENDIF
        IF(simulate%substance(i_t2))THEN
          CALL calculate_class_outvar_initialize(o_crunT2,i)
          CALL calculate_class_outvar_add(o_crunT2,i,1.,crunofftsi(i_t2))
        ...
        IF(conduct%simN)THEN
          ...
          CALL calculate_class_outvar_initialize(o_crunON,i)
          CALL calculate_class_outvar_initialize(o_crunTN,i)
          CALL calculate_class_outvar_add(o_crunIN,i,1.,crunofftsi(i_in))
          CALL calculate_class_outvar_add(o_crunON,i,1.,crunofftsi(i_on))
          CALL calculate_class_outvar_add(o_crunTN,i,1.,crunofftsi(i_in)+crunofftsi(i_on))
          CALL calculate_class_outvar_finish(o_crunIN,i,runofftsi*1.E-3)  !IN conc in runoff [ug/L]
          CALL calculate_class_outvar_finish(o_crunON,i,runofftsi*1.E-3)  !ON conc in runoff [ug/L]
          CALL calculate_class_outvar_finish(o_crunTN,i,runofftsi*1.E-3)  !TN conc in runoff [ug/L]
        ENDIF
        IF(conduct%simP)THEN
          CALL calculate_class_outvar_initialize(o_crunSP,i)
          CALL calculate_class_outvar_initialize(o_crunPP,i)
          CALL calculate_class_outvar_initialize(o_crunTP,i)
          CALL calculate_class_outvar_add(o_crunSP,i,1.,crunofftsi(i_sp))
          CALL calculate_class_outvar_add(o_crunPP,i,1.,crunofftsi(i_pp))
          CALL calculate_class_outvar_add(o_crunTP,i,1.,crunofftsi(i_sp)+crunofftsi(i_pp))
          CALL calculate_class_outvar_finish(o_crunSP,i,runofftsi*1.E-3)  !SP conc in runoff [ug/L]
          CALL calculate_class_outvar_finish(o_crunPP,i,runofftsi*1.E-3)  !PP conc in runoff [ug/L]
          CALL calculate_class_outvar_finish(o_crunTP,i,runofftsi*1.E-3)  !TP conc in runoff [ug/L]
        ENDIF
        IF(simulate%substance(i_oc))THEN
          CALL calculate_class_outvar_initialize(o_crunOC,i)
          CALL calculate_class_outvar_add(o_crunOC,i,1.,crunofftsi(i_oc))
          CALL calculate_class_outvar_finish(o_crunOC,i,runofftsi)  !OC conc in runoff [mg/L]
        ENDIF
        IF(simulate%substance(i_ss))THEN
          CALL calculate_class_outvar_initialize(o_crunSS,i)
          CALL calculate_class_outvar_add(o_crunSS,i,1.,crunofftsi(i_ss))
      ...
      IF(simulate%substance(i_t2)) ctemp_T2 = cin(i_t2)  !Keep T2 concentration in memory, in case water is being extracted/added local sources
      ...
      IF(simulate%substance(i_t2)) cin(i_t2) = ctemp_T2 !Restore T2 concentration, in case water was added by local sources
      IF(conduct%simT1.OR.conduct%simT2) CALL add_tracer_point_source_to_river(i,itype,qin,cin)  !T1 and T2 point source
      IF(simulate%substance(i_t2)) ctemp_T2 = cin(i_t2)  !Keep T2 concentration in memory, in case water is being influenced by wetland
      ...
      IF(riverlength(itype,i)>0.)THEN
        ...
        IF(simulate%substance(i_t2)) cin(i_t2) = ctemp_T2 !Restore T2 concentration, in case initial T2 of wetland will influence
        ...
        CALL calculate_river_characteristics(i,itype,transq,conduct%simN.OR.conduct%simP.OR.conduct%simS.OR.(i_t1>0),riverstate,riverdepth,riverarea(itype),riverQbank)
        ...
        IF(a>0)THEN      !river has classarea
          ...
          CALL calculate_class_outvar_add(o_snowfall,i,a,snowfall)
          corrpreci = corrpreci + prec*a
          CALL calculate_class_outvar_add(o_epot,i,a,epot*riverfracarea(itype))
          eacti = eacti + evapr*a*riverfracarea(itype)
          IF(simulate%substance(i_t1)) CALL calculate_class_outvar_add(o_cevapT1,i,a,cevaprT1*evapr*riverfracarea(itype))
          icpevapi = icpevapi + icpevap*a

          !Set water balance output
          IF(conductwb)THEN
            wbflows(w_piriver,i) = prec * riverarea(itype)*1.E-3     !m3
            wbflows(w_eiriver,i) = evapr * riverfracarea(itype) * riverarea(itype)*1.E-3
          ENDIF
        ENDIF !(a>0)    
      ...
      IF(a>0)THEN      !local lake exist
        ...
        eacti = eacti + evapl*a
        IF(simulate%substance(i_t1)) CALL calculate_class_outvar_add(o_cevapT1,i,a,cevapl(i_t1)*evapl)
        icpevapi = icpevapi + icpevap*a
        IF(conductload)THEN
        ...
        IF(simulate%substance(i_t2)) CALL calculate_lake_epilimnion_depth(lakearea(itype),prec,evapl,qinmm,epidepth)
      ...
      IF(riverlength(itype,i)>0.)THEN
        ...
        IF(a>0)THEN   !river has area
          ...
          CALL calculate_class_outvar_add(o_snowfall,i,a,snowfall)
          corrpreci = corrpreci + prec*a
          CALL calculate_class_outvar_add(o_epot,i,a*(1.-fpfrac),epot * riverfracarea(itype))
          eacti = eacti + (evapr * (1.-fpfrac) * riverfracarea(itype)) * a
          IF(simulate%substance(i_t1)) CALL calculate_class_outvar_add(o_cevapT1,i,a*(1.-fpfrac),cevaprT1*evapr*riverfracarea(itype))
        ...
        IF(modeloption(p_floodplain)>=3)THEN
          ...
          IF(dampq<0.)THEN
            ...
            !WRITE(6,*) 'River flow dammed to zero for subbasin isub',i
          ENDIF
        ENDIF
        IF(numsubstances>0) dampc = riverstate%conc(:,itype,i)
        CALL remove_water(riverstate%water(itype,i),numsubstances,riverstate%conc(:,itype,i),dampq,dampc,status)
      ...
      IF(a>0)THEN
        ...
        IF(simulate%substance(i_t1)) CALL calculate_class_outvar_add(o_cevapT1,i,a*(1.-fpfrac),cevapl(i_t1)*evapl)
        icpevapi = icpevapi + icpevap*(1.-fpfrac)*a
        ...
        IF(modeloption(p_floodplain)>=3)THEN
          IF(downstreamdammedflowonolake(i)<0.)THEN
            outflowmm = outflowmm + downstreamdammedflowonolake(i)/lakearea(itype)*1000.
            wthresh = basin(i)%lakedepth(itype)*1000.
            IF(outflowmm<0.)THEN
              outflowmm = 0.  !Catch negative outflow
              !WRITE(6,*) 'Lake outflow dammed to zero for subbasin isub',i
            ELSEIF(lakewst(itype)>wthresh)THEN
              IF(outflowmm>lakewst(itype)-wthresh)THEN  
                outflowmm = lakewst(itype) - wthresh
              ENDIF
              IF(outflowmm>lakewst(itype)) outflowmm = lakewst(itype)   !Safety 
            ELSE
              outflowmm = 0.
            ENDIF
            ...
            !Lägg till check för sådant i inläsning.
          ENDIF
        ENDIF
        lakeoutflow(itype) = outflowm3s
        CALL remove_outflow_from_lake(i,itype,numsubstances,outflowmm,basin(i)%subid,basin(i)%lakedepth(2),epidepth,lakewst(itype),concout,lakestate) !remove water and mixes lake
      ...
      IF(simulatesubstances) clakeoutflow(:,itype) = concout
      ...
      IF(lakeoutflow(2)>0)THEN
        IF(mainflow>0 .AND. path(i)%main>0)THEN
          ...
          IF(simulatesubstances) acccinflow(:,path(i)%main) = acccinflow(:,path(i)%main) + clakeoutflow(:,2) * mainflow
        ENDIF
        IF(branchflow>0)THEN
          IF(branchdata(branchindex(i))%branch>0)THEN
            ...
            IF(simulatesubstances) acccinflow(:,branchdata(branchindex(i))%branch) = acccinflow(:,branchdata(branchindex(i))%branch) + clakeoutflow(:,2) * branchflow
          ENDIF
        ENDIF
      ENDIF
      ...
      ELSE
        ...
        ENDDO
        CALL calculate_class_outvar_finish(o_soilden3,i,0.)
        CALL calculate_class_outvar_finish(o_soildenrz,i,0.)
        DO k=305,322
          CALL set_class_outvar_missing(k,i)
        ENDDO
      ENDIF
      ...
      IF(simulate%substance(i_in).AND.outvarindex(55)>0) outvar(i,outvarindex(55)) = clakeoutflow(i_in,2)*1000.   !comp conc IN outflow olake (ug/l)
      IF(simulate%substance(i_on).AND.outvarindex(56)>0) outvar(i,outvarindex(56)) = clakeoutflow(i_on,2)*1000.   !comp conc ON outflow olake (ug/l)
      IF(simulate%substance(i_sp).AND.outvarindex(57)>0) outvar(i,outvarindex(57)) = clakeoutflow(i_sp,2)*1000.   !comp conc SRP outflow olake (ug/l)
      IF(simulate%substance(i_pp).AND.outvarindex(58)>0) outvar(i,outvarindex(58)) = clakeoutflow(i_pp,2)*1000.   !comp conc PP outflow olake (ug/l)
      CALL set_outvar_xobs(62,i)  !rec T1 conc in outflow
      IF(simulate%substance(i_t1).AND.outvarindex(63)>0) outvar(i,outvarindex(63)) = clakeoutflow(i_t1,2)                        !comp conc T1 conc in outflow
      IF(simulate%substance(i_t2).AND.outvarindex(64)>0) outvar(i,outvarindex(64)) = clakeoutflow(i_t2,2)                        !comp conc T2 conc in outflow
      ...
      IF(simulate%substance(i_in).AND.outvarindex(102)>0) outvar(i,outvarindex(102)) = clakeoutflow(i_in,1)*1000.
      IF(simulate%substance(i_on).AND.outvarindex(103)>0) outvar(i,outvarindex(103)) = clakeoutflow(i_on,1)*1000.
      IF(simulate%substance(i_sp).AND.outvarindex(104)>0) outvar(i,outvarindex(104)) = clakeoutflow(i_sp,1)*1000.
      IF(simulate%substance(i_pp).AND.outvarindex(105)>0) outvar(i,outvarindex(105)) = clakeoutflow(i_pp,1)*1000.
      ...
      IF(simulate%substance(i_oc).AND.outvarindex(135)>0) outvar(i,outvarindex(135)) = clakeoutflow(i_oc,2)*flow1000m3ts  !TOC load (kg/timestep)
      IF(simulate%substance(i_oc).AND.outvarindex(203)>0) outvar(i,outvarindex(203)) = clakeoutflow(i_oc,1)    !orgC in flow from local river
      DO k=0,9
      ...
      IF(conductload) THEN
        ...
        outvar_basinload(:,8:20,i)          = Lpathway(:,1:13)
        outvar_basinload(:,21,i)            = Lbranch(:)
      ENDIF
  ...
  SUBROUTINE calculate_class_outvar_finish_scale(idindex,isb,fraction)
    ...
    INTEGER, INTENT(IN) :: idindex  !<index in outvarid
    INTEGER, INTENT(IN) :: isb      !<index of subbasin
    REAL, INTENT(IN)    :: fraction !<land area fraction of subbasin area (or other factor)
  ...
  !>Set subbasin mean value from classes to missing in case used when should not
  !>\b Consequences Module modvar variable outvar may change
  !--------------------------------------------------------------------
