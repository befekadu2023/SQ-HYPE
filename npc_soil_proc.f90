...
!        sourceN = deposition%indryload(i,veg)
        fakeflow = 1.
...
!        sourceP = drypp
  ...
  SUBROUTINE calculate_plant(i,j,dayno,temp,daylength,common_uptake,miscstate)
    ...
    REAL bd2,bd3,bd5                    !cultivation parameters
    ...
    DO kcrop = 1, 2                             !main and secondary crop
      ...
      IF(part>0)THEN
        ...
        !uptakeN = up1*up2*up3*help/(up2+help)/(up2+help)!potential uptake of N, from sowing to autumn ploughing
        uptakeN = plant_growth(cropdata(k)%uptake1,cropdata(k)%uptake2,cropdata(k)%uptake3,REAL(dayno-bd2))
        IF(bd5==0)THEN
          IF(bd3>bd2)THEN
            IF(dayno<bd2.OR.dayno>bd3) uptakeN = 0.
          ELSE    !crop over winter
            IF(dayno>bd3.AND.dayno<bd2) uptakeN = 0.
          ENDIF
        ELSE
          IF(dayno<bd2) uptakeN = 0.
          IF(dayno>bd3) uptakeN = 0.
          IF(dayno>bd5) THEN        !autumn crops grow after autumn sowing
            IF(temp<5.) THEN       
              tmpfcn=0.
            ELSE
        ...
        !IF(dayno>bd3) THEN
        !  uptakeN = 0.
        !  IF(bd5>0 .AND. dayno>bd5) THEN        !autumn crops grow after autumn sowing
        !    IF(temp<5.) THEN       
        !      tmpfcn=0.
        !    ELSE
        !      tmpfcn = min(1., (1./20.)*temp-(5./20.)) 
  ...
  SUBROUTINE particle_processes_for_runoff(i,j,isoil,iluse,dayno,prec,surfacerunoff,macroporflow,    &
    ...
    IF(simulate%substance(i_pp)) CALL calculate_pp_transport(erodingflow,erodedsed,soilstate%partP(1,j,i),soilstate%humusP(1,j,i),soilthick(1,j),fracminP,erodedP)
