...
600 FORMAT(A32,I7,A100)
  ...
  SUBROUTINE check_station(funit,infile,ns,nskip,geostn,nobsstn,oindex,status) 
    ...
    DO i = 1,nskip  !Skip comment rows
      READ(funit,*,ERR=201)
    ENDDO    
    READ(funit,*,ERR=201) str,stn(1:nobsstn)
    CLOSE(funit)
...
201 WRITE(6,*) 'Error: reading file',TRIM(infile)
  ...
  SUBROUTINE check_q_stn(funit,infile,ncols,geostn,nobsstn,qindex,status) 
    ...
    IF(nobsstn>ncols)THEN
      IF(conductwarning) WRITE(6,*) 'WARNING:'
      IF(conductwarning) WRITE(6,*) 'WARNING: More stations in Qobs.txt than subbasins in GeoData.txt'
      IF(conductwarning) WRITE(6,*) 'WARNING: Only the',ncols,'first stations is read'
      nobsstn=ncols
    ENDIF
    BACKSPACE(funit)
    ...
    IF(SUM(qindex)==0)THEN
       WRITE(6,*) 
       IF(conductwarning) WRITE(6,*) 'WARNING: no Q-stations found in '//TRIM(infile)//' which are in '
       IF(conductwarning) WRITE(6,*) 'WARNING: accordance with catchments in GeoData.txt'
       status = 3
       RETURN
    ENDIF
