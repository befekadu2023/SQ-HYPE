...
MODULE IRRIGATION_MODULE
  ...
  !Copyright 2011-2015,2017-2018 SMHI
  !
...
CONTAINS
  ...
  SUBROUTINE calculate_irrigation_water_demand(i,j,dn,area,sswcorrpar,immdeppar,  &
    ...
    IF(cropiindex==0)THEN   !Temporary tests. These should be checked in the beginning, not here every time.
      ...
      STOP 1
    ENDIF
!    IF(irrindex(i)==0)THEN   !This check is not working with WWH-files, therefore removed (if irrindex=0->irrigation_season=false)
!      WRITE(6,*) 'ERROR: Subbasin ',basin(i)%subid, 'with irrigated class',j
!      WRITE(6,*) 'ERROR: is not found in MgmtData'
!      STOP 1
!    ENDIF
