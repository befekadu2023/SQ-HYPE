...
!> \{
  ...
  INTEGER, PARAMETER :: max_basinoutvar = 21    !<Number of basin load output variables
...
!> \brief Type for holding information about soil leakage "forcing" data
  TYPE SOILLEAKAGEDATATYPE
    ...
    !INTEGER          :: ncols       !<number of columns of data (nsub än så länge)
...
!> \brief Type for holding information about output variables
  TYPE OUTVARIDTYPE
    ...
    INTEGER           :: basearea       !<base area (0=undefined,1=basinarea,2=landarea,3-5=no area (scaling 1000))
...
!> \{
  ...
  INTEGER, PARAMETER :: p_wetland        = 14 !Wetland model(s?) (0) no wetland model, (1) river wetlands
  INTEGER, PARAMETER :: p_soilleakage    = 15 !Soil leakage concentration model (0) calculate soil concentrations and leakage, (1) read monthly soil leakage from file
  INTEGER, PARAMETER :: num_modelprocess = 15 !Number of processes with options in HYPE
  CHARACTER(LEN=16),ALLOCATABLE :: modeloptionname(:)
  INTEGER,ALLOCATABLE :: modeloption(:)
  
  INTEGER :: maxmodelsinoption(num_modelprocess)
  PARAMETER(maxmodelsinoption = [2,3,3,6,3,2,2,2,3,2,2,2,2,2,2])
  ...
  PARAMETER(infiltrationmodelnames = [CHARACTER(LEN=64) :: 'current HYPE', 'restricted infiltration into frozen soils'])
  ...
  PARAMETER(wetlandmodelnames = [CHARACTER(LEN=64) :: 'no wetland model', 'river wetlands'])
!> \}
...
!    conduct%simT = .FALSE.
  ...
  !----------------------------------------------------------------------------------
  SUBROUTINE set_soildepth_classdata(nlayer,depth,classdata)

    INTEGER, INTENT(IN) :: nlayer         !<number of soil layers
    REAL, INTENT(IN)    :: depth(nlayer)  !<soil layer depth (m)
    TYPE(CLASSTYPE) :: classdata      !<variable holding class characteristics

    classdata%soildepth(1:nlayer) = depth
    IF(nlayer<maxsoillayers) classdata%soildepth(nlayer+1:maxsoillayers) = classdata%soildepth(nlayer)
  ...
  INTEGER FUNCTION set_soildepth(nlayer,slc,depth)
    ...
    set_soildepth = 0
    IF(.NOT.ALLOCATED(soildepth))THEN
      ALLOCATE(soildepth(maxsoillayers,nclass))
      soildepth = 0.
    ENDIF
    IF(.NOT.ALLOCATED(soilthick))THEN
      ALLOCATE(soilthick(maxsoillayers,nclass))
      soilthick = 0.
    ENDIF
  ...
  !------------------------------------------------------------------------------
  FUNCTION get_current_soil_leakage(isub,nsubst) RESULT(concentration)

    !Argument declarations
    INTEGER, INTENT(IN)  :: isub         !<current subbasin
    INTEGER, INTENT(IN)  :: nsubst       !<number of substances
    REAL :: concentration(nsubst)        !<current soil leakage concentration
    
    !>Monthly soilleakage concentrations
    IF(modeloption(p_soilleakage)==1)THEN
      concentration = soilleak%concentration(:,month,isub)
    !Or read other soilleakage
    !ELSEIF(modeloption(p_soilleakage)==2)THEN
    ENDIF

  END FUNCTION get_current_soil_leakage
  ...
  SUBROUTINE set_update_nutrientcorr(dim,ns,nrow,icorr,dataarray,updarray)
    ...
    IF(.NOT.ANY(updarray/=0))THEN
      IF(conductwarning) WRITE(6,*) 'WARNING: missing data in update.txt file, no ',TRIM(updatename(icorr)),' updating simulated'
