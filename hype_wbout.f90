...
!> \{
  ...
  INTEGER,PARAMETER :: w_wtrtomr   = 65  !<Index for water transfer from outlet lake
  INTEGER,PARAMETER :: w_wtrofol   = 66  !<Index for water transfer to main river
  INTEGER,PARAMETER :: num_wbflows = 66  !<Number of water balance flows
!>\}
...
!> \{
  ...
  INTEGER,PARAMETER :: w_wtcanal    = 14  !<Index for water transfer canal
  INTEGER,PARAMETER :: num_wbstores = 14  !<Number of water balance stores
!>\}
...
! !Variables for file names and file units, and for which subbasins has irrigation.
  ...
  CHARACTER(LEN=50) :: fname_wbflows(num_wbflows) = (/ &
      ...
      'WBf_watertransfer_olake_wtcanal.txt             '/) !<filenames waterbalance flows
