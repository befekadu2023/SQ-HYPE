...
!                      timesteps_per_day, &
                      ...
                      conduct,simulate, &
                      simulatesubstances, &
                      p_snowmelt,p_snowevap,p_soilleakage
  ...
  REAL :: glac_areavol_par(2,2)              !<variable holding parameters for glacier area-volume relationship
...
!    REAL trunofftemp(maxsoillayers)
    ...
    CALL calculate_soil_runoff(i,j,subid,wpmm(:,j),fcmm(:,j),epmm(:,j),classdata(j)%streamdepth,soilstate,soilrunoff,csoilrunoff)
    ...
    CALL soil_np_processes(i,j,iluse,conduct%simN,conduct%simP,conduct%simC,dayno,classarea,wpmm(:,j),fcmm(:,j),epmm(:,j),plantuptake,soilthick(:,j),genpar(m_fertdays),genpar(m_littdays),  &
    ...
    CALL balance_spsoil(i,j,conduct%simP,soilthick(:,j), & 
