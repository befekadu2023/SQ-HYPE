...
MODULE SURFACEWATER_PROCESSES
  ...
  SUBROUTINE calculate_actual_lake_evaporation(i,j,itype,numsubst,temp,epot,evap,cevap,lakestate)
    ...
    IF(temp>tt .AND. epot>0) THEN
      ...
      IF(evap+realzero<lakestate%water(itype,i))THEN
        IF(numsubst>0) CALL set_evaporation_concentrations(lakestate%conc(:,itype,i),cevap)
      ...
      ELSEIF(lakestate%slowwater(itype,i)>0.)THEN
        !lake divided in two parts for substance simulations
        !mix lake parts and put in slowlake-part, evaporation from mixed lake volume
        ...
        IF(numsubst>0) CALL set_evaporation_concentrations(lakestate%concslow(:,itype,i),cevap)
        IF(evap+realzero<lakestate%slowwater(itype,i))THEN
        ...
        ELSE
          ...
          lakestate%slowwater(itype,i) = 0.
          lakestate%concslow(:,itype,i) = 0.
        ENDIF
      ELSE
        !no water in slowlake-part, empty the lake
        evap = lakestate%water(itype,i)
    ...
    DO
      ...
      IF(absvol<lakevol)THEN
        IF(ALLOCATED(lakestate%slowwater))THEN
          ...
          lakestate%water(pooltype,i) = lakestate%water(pooltype,i) - absvol*waterfrac
          lakestate%slowwater(pooltype,i) = lakestate%slowwater(pooltype,i) - absvol*(1.-waterfrac)
          IF(numsubstances>0) absconc = waterfrac*lakestate%conc(:,pooltype,i) + (1.-waterfrac)*lakestate%concslow(:,pooltype,i)
        ELSE
          lakestate%water(pooltype,i) = lakestate%water(pooltype,i) - absvol
          IF(numsubstances>0) absconc = lakestate%conc(:,pooltype,i)  !not used
        ENDIF
        !watertaken = watertransfer(wtindex)%flow   !m3/s
        watertaken = absvol / qunitfactor 
      ELSE
        IF(numsubstances>0) absconc = (lakestate%water(pooltype,i)*lakestate%conc(:,pooltype,i) + lakestate%slowwater(pooltype,i)*lakestate%concslow(:,pooltype,i))/lakevol
        lakestate%water(pooltype,i) = 0.
        IF(ALLOCATED(lakestate%slowwater)) lakestate%slowwater(pooltype,i) = 0.
  ...
  SUBROUTINE calculate_ilake_outflow(i,subid,ns,dividedlake,qin,lakearea,qunitfactor, &
    ...
    !>Remove outflow from lake and mix fast and slow lake parts
    ...
    ELSE
      IF(lakedatapar(lakedataparindex(i,itype),m_lddeeplake)==0)THEN
        ...
        IF(outflowmm>0)THEN
          ...
          IF(outflowmm+realzero<=lakestate%slowwater(itype,i))THEN
          ...
          ELSE
            lakestate%slowwater(itype,i) = 0.
            IF(ns>0) lakestate%concslow(:,itype,i)=0.
          ENDIF
        ENDIF
      ELSE
        ...
        IF(outflowmm>0)THEN
          ...
          IF(q1>0.)THEN
            ...
            IF(q1+realzero<=lakestate%water(itype,i))THEN
          ...
          IF(q2>0)THEN 
            ...
            IF(usestrattemp) tempconc(i_t2) = t2conc
            IF(q2+realzero<=lakestate%slowwater(itype,i))THEN
  ...
  SUBROUTINE calculate_flow_within_lake(i,itype,subid,lakestate)
    ...
    IF(lakedatapar(lakedataparindex(i,itype),m_lddeeplake)>0.)THEN
      ...
      ELSEIF(lakestate%water(itype,i)>0.AND.slowlakeini(itype,i)-lakestate%slowwater(itype,i)>0)THEN    !move all lakewi-water to slowlake
        ...
        lakestate%conc(:,itype,i)=0.
      ENDIF
    !If deeplake=0 the lake is already totally mixed (when calculated outflow)
    ENDIF
  ...
  SUBROUTINE calculate_branched_flow(i,totflow,mainflow,branchflow)
    ...
    IF(.NOT.branchdata(branchindex(i))%recQbranch)THEN
      mainflow = totflow
...
!
  ...
  SUBROUTINE T2_processes_in_lake(i,itype,temp,swrad,lakesurft,lakearea,epidepth,frozenstate,lakestate,freezeup,freezeuparea)
    ...
    !Mix lake T2 concentration regardless of lake distribution type for other substances
    meanwater = lakestate%water(itype,i)+lakestate%slowwater(itype,i)
    IF(meanwater.GT.0.)THEN
      meantemp  = (lakestate%water(itype,i)*lakestate%conc(i_t2,itype,i)+lakestate%slowwater(itype,i)*lakestate%concslow(i_t2,itype,i))/meanwater
      lakestate%conc(i_t2,itype,i) = meantemp
        ...
        IF(itype==2 .AND. epidepth < meanwater*0.001 .AND. epidepth>0.)THEN !why is this only possible for olakes?
          ...
          IF(icefreefraction.GT.0.)THEN
            ...
            !->re-calculate meantemp
             meantemp = (lakestate%uppertemp(itype,i) * epidepth + lakestate%lowertemp(itype,i) * (meanwater * 0.001 - epidepth)) / (meanwater * 0.001) 
...
!          SELECT CASE(modeloption(p_lakeriverice))
!          CASE(2) ! new model based on Piccolroaz et al 2013
!            ! Apply on uppertemp (see comment above for non-splitted lakes) DG20180311
